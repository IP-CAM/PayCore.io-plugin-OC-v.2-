...
class ControllerPaymentPaycore extends Controller {
    ...
    public function index() {
        ...
        if (version_compare(VERSION, '2.2.0.0', '<')) {
            return is_file(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/paycore.tpl')
                    ? $this->load->view($this->config->get('config_template') . '/template/payment/paycore.tpl', $this->data)
                    : $this->load->view('default/template/payment/paycore.tpl', $this->data);
        } else {
            return $this->load->view('payment/paycore.tpl', $this->data);
        }
    }
